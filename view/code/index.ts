...
function getOutputAriaLabel(value: OutputFormats): string {
  if (value === 'numbers') {
    return `Valores numéricos em ${LCH ? 'LCH' : 'OKLCH'}`
  } else if (value === 'lrgb') {
    return 'Código Linear RGB'
  } else if (value === 'figmaP3') {
    return 'Código Figma P3'
  } else if (value === 'hex/rgba') {
    return 'Código CSS em HEX ou RGBA'
  } else {
    return `Código CSS em ${value.toUpperCase()}`
  }
}
...
function listenChanges(input: HTMLInputElement): void {
  function processChange(): void {
    ...
    if (!setCurrent(newValue, input === rgbInput)) {
      setInvalid(input, 'Use um formato de cor CSS válido')
    }
  }
...
outputFormat.subscribe(value => {
  format.value = value
  rgbInput.ariaLabel = getOutputAriaLabel(value)
})
outputFormat.listen(() => {
  setRgb()
})
